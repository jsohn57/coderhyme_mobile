...
try{
	dCasperSports.start("http://media.daum.net/netizen/popular/?include=entertain");
	...
	dCasperSports.run(function () {
		...
		if(fs.exists("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_ent_article_url.in")){
			fs.remove("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_ent_article_url.in");
		}	
		var articleStream = fs.open("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_ent_article_url.in", 'w');
		for(i  = 0; i < il; i++){
			articleStream.write(pageInfo[i]['article_url'] + '\n');
		}
		articleStream.close();

		if(fs.exists("/projects/GiantEagle/webcrawler/dat/crwl/daum_ent.crwl")){
			fs.remove("/projects/GiantEagle/webcrawler/dat/crwl/daum_ent.crwl");
		}
		fs.write("/projects/GiantEagle/webcrawler/dat/crwl/daum_ent.crwl", JSON.stringify({
			channel : 'daum_ent',
			at      : new Date(),
			data    : completeList
			}), 'w');

		this.echo("Done writing /projects/GiantEagle/webcrawler/dat/crwl/daum_ent.crwl");
