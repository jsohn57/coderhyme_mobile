...
try{
	...
	dCasperSports.run(function () {
		...
		var fs = require('fs');
		if(fs.exists("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_pol_article_url.in")){
      fs.remove("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_pol_article_url.in");
    }
		if(fs.exists("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_eco_article_url.in")){
      fs.remove("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_eco_article_url.in");
    }
		if(fs.exists("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_soc_article_url.in")){
      fs.remove("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_soc_article_url.in");
    }
    var articleStream = fs.open("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_pol_article_url.in", 'w');
    for(i  = 0, il = polList.length; i < il; i++){
      articleStream.write(polList[i]['article_url'] + '\n');
    }
		articleStream.close();
    articleStream = fs.open("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_eco_article_url.in", 'w');
    for(i  = 0, il = ecoList.length; i < il; i++){
      articleStream.write(ecoList[i]['article_url'] + '\n');
    }
		articleStream.close();
    articleStream = fs.open("/projects/GiantEagle/webcrawler/daum/mobile_crwl/m_daum_soc_article_url.in", 'w');
    for(i  = 0, il = socList.length; i < il; i++){
      articleStream.write(socList[i]['article_url'] + '\n');
    }
		articleStream.close();
