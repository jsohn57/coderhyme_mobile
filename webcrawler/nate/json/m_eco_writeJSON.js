...
var fs = require('fs');
var stream, i, il;
stream = fs.open("/projects/GiantEagle/webcrawler/dat/nate/m_eco/article_url.in", 'r');
for(i = 0, il = NUM_ARTICLES; i < il; i++){
	articleURLList.push(stream.readLine());
}
stream.close();
stream = fs.open("/projects/GiantEagle/webcrawler/dat/nate/m_eco/title.in", 'r');
for(i = 0, il = NUM_ARTICLES; i < il; i++){
...
try{
	...
	nCasperSports.run(function() {
		...
		if(fs.exists("/projects/GiantEagle/webcrawler/dat/crwl/m_nate_eco.crwl")){
			fs.remove("/projects/GiantEagle/webcrawler/dat/crwl/m_nate_eco.crwl");
		}	

		fs.write("/projects/GiantEagle/webcrawler/dat/crwl/m_nate_eco.crwl", JSON.stringify({
			channel : 'm_nate_eco',
			at      : new Date(),
			data    : completeList
			}), 'w');

		this.echo("Done writing /projects/GiantEagle/webcrawler/dat/crwl/m_nate_eco.crwl");
		this.exit(0);
	});
}
