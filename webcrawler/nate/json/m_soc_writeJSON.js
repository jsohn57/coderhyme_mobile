...
var fs = require('fs');
var stream, i, il;
stream = fs.open("/projects/GiantEagle/webcrawler/dat/nate/m_soc/article_url.in", 'r');
for(i = 0, il = NUM_ARTICLES; i < il; i++){
	articleURLList.push(stream.readLine());
}
stream.close();
stream = fs.open("/projects/GiantEagle/webcrawler/dat/nate/m_soc/title.in", 'r');
for(i = 0, il = NUM_ARTICLES; i < il; i++){
...
try{
	...
	nCasperSports.run(function() {
		...
		if(fs.exists("/projects/GiantEagle/webcrawler/dat/crwl/m_nate_soc.crwl")){
			fs.remove("/projects/GiantEagle/webcrawler/dat/crwl/m_nate_soc.crwl");
		}	

		fs.write("/projects/GiantEagle/webcrawler/dat/crwl/m_nate_soc.crwl", JSON.stringify({
			channel : 'm_nate_soc',
			at      : new Date(),
			data    : completeList
			}), 'w');

		this.echo("Done writing /projects/GiantEagle/webcrawler/dat/crwl/m_nate_soc.crwl");
		this.exit(0);
	});
}
